WITH cohort AS (
    SELECT
        ...
        n.state,
        n.reason_code,
        DATEDIFF('SECOND', n.created_at, n.updated_at) / 86400.0 AS days_to_resolve
...
daily AS (
    SELECT
        ...
        COUNT(DISTINCT CASE
            ...
            AND days_to_resolve <= 21
            THEN EXECUTION_CANDIDATE_ID
        END) AS resolved,
        ...
        COUNT(DISTINCT CASE
            WHEN state = 'CANCELLED'
                 AND reason_code = 'DEVICE_RESCUED'
                 AND days_to_resolve <= 21
            THEN EXECUTION_CANDIDATE_ID
        END) AS recharge_done,
...
FROM (
    SELECT
        created_dt,
        '1. Resolved within 21 days %' AS metric,
        1 AS sort_order,
        ROUND(100.0 * resolved / NULLIF(total, 0), 1) AS val
    FROM daily
    ...
    SELECT
        created_dt,
        '↳ Pickup Done %' AS metric,
        2 AS sort_order,
        ROUND(100.0 * pickup_done / NULLIF(total, 0), 1) AS val
    FROM daily
    ...
    SELECT
        created_dt,
        '↳ Recharge Done %' AS metric,
        3 AS sort_order,
        ROUND(100.0 * recharge_done / NULLIF(total, 0), 1) AS val
    FROM daily
    ...
    SELECT
        created_dt,
        '2. Lost Rate %' AS metric,
        4 AS sort_order,
        ROUND(100.0 * lost / NULLIF(total, 0), 1) AS val
    FROM daily
) x
