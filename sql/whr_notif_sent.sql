...
taskc AS (SELECT DISTINCT connection_id FROM PROD_DB.CSP_TAS_SERVICE_CSP_TAS_SERVICE.INSTALL_EXECUTION_CANDIDATES),
...
ev AS (
  ...
  WHERE e.timestamp >= DATE_TRUNC('month', DATEADD('month',-3,CURRENT_DATE())) AND e.event_name='install_task_created'
  GROUP BY 1
),
f AS (
  ...
  FROM uni u
  JOIN taskc t ON t.connection_id=u.connection_id
  LEFT JOIN cand c ON c.connection_id=u.connection_id
  LEFT JOIN ev ON ev.eid=c.execution_candidate_id
),
fd AS (SELECT DISTINCT connection_id,cdate,pn_sent FROM f)
SELECT 'Booking-to-Task Notif Sent Rate %' "KPI",
 ROUND(SUM(CASE WHEN cdate=w.d1 THEN pn_sent END)*100.0/NULLIF(SUM(CASE WHEN cdate=w.d1 THEN 1 END),0),2) "D-1",
 ROUND(SUM(CASE WHEN cdate=w.d2 THEN pn_sent END)*100.0/NULLIF(SUM(CASE WHEN cdate=w.d2 THEN 1 END),0),2) "D-2",
 ROUND(SUM(CASE WHEN cdate=w.d3 THEN pn_sent END)*100.0/NULLIF(SUM(CASE WHEN cdate=w.d3 THEN 1 END),0),2) "D-3",
 ROUND(SUM(CASE WHEN cdate BETWEEN w.w1f AND w.w1t THEN pn_sent END)*100.0/NULLIF(SUM(CASE WHEN cdate BETWEEN w.w1f AND w.w1t THEN 1 END),0),2) "W-1",
 ROUND(SUM(CASE WHEN cdate BETWEEN w.w2f AND w.w2t THEN pn_sent END)*100.0/NULLIF(SUM(CASE WHEN cdate BETWEEN w.w2f AND w.w2t THEN 1 END),0),2) "W-2",
 ROUND(SUM(CASE WHEN cdate BETWEEN w.w3f AND w.w3t THEN pn_sent END)*100.0/NULLIF(SUM(CASE WHEN cdate BETWEEN w.w3f AND w.w3t THEN 1 END),0),2) "W-3",
 ROUND(SUM(CASE WHEN cdate BETWEEN w.m1f AND w.m1t THEN pn_sent END)*100.0/NULLIF(SUM(CASE WHEN cdate BETWEEN w.m1f AND w.m1t THEN 1 END),0),2) "M-1",
 ROUND(SUM(CASE WHEN cdate BETWEEN w.m2f AND w.m2t THEN pn_sent END)*100.0/NULLIF(SUM(CASE WHEN cdate BETWEEN w.m2f AND w.m2t THEN 1 END),0),2) "M-2",
 ROUND(SUM(CASE WHEN cdate BETWEEN w.m3f AND w.m3t THEN pn_sent END)*100.0/NULLIF(SUM(CASE WHEN cdate BETWEEN w.m3f AND w.m3t THEN 1 END),0),2) "M-3"
