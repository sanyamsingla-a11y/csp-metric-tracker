...
agg AS (
    SELECT p.period,
        ROUND(SUM(CASE WHEN d.entry_type IN ('BASE_PAYOUT','BONUS_CREDIT','RECOVERY_RETURN') THEN d.AMOUNT ELSE 0 END) / 100.0, 2) AS numerator_amt,
        ROUND(SUM(d.AMOUNT) / 100.0, 2) AS total_amt
    FROM all_data d
...
SELECT
    'BASE+BONUS+RECOVERY % of TOTAL' AS metric,
    MAX(CASE WHEN period='D-1' THEN ROUND(100.0 * numerator_amt / NULLIF(total_amt, 0), 2) END) AS "D-1",
    MAX(CASE WHEN period='D-2' THEN ROUND(100.0 * numerator_amt / NULLIF(total_amt, 0), 2) END) AS "D-2",
    MAX(CASE WHEN period='D-3' THEN ROUND(100.0 * numerator_amt / NULLIF(total_amt, 0), 2) END) AS "D-3",
    MAX(CASE WHEN period='W-1' THEN ROUND(100.0 * numerator_amt / NULLIF(total_amt, 0), 2) END) AS "W-1",
    MAX(CASE WHEN period='W-2' THEN ROUND(100.0 * numerator_amt / NULLIF(total_amt, 0), 2) END) AS "W-2",
    MAX(CASE WHEN period='W-3' THEN ROUND(100.0 * numerator_amt / NULLIF(total_amt, 0), 2) END) AS "W-3",
    MAX(CASE WHEN period='M-1' THEN ROUND(100.0 * numerator_amt / NULLIF(total_amt, 0), 2) END) AS "M-1",
    MAX(CASE WHEN period='M-2' THEN ROUND(100.0 * numerator_amt / NULLIF(total_amt, 0), 2) END) AS "M-2",
    MAX(CASE WHEN period='M-3' THEN ROUND(100.0 * numerator_amt / NULLIF(total_amt, 0), 2) END) AS "M-3"
FROM agg
