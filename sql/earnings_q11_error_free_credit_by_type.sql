with params as (select convert_timezone('Asia/Kolkata',current_timestamp())::date as as_of_date),
...
intervention_events as (
    select 'INTERVENTION_CREDIT' event_type, e.correlation_id, date(convert_timezone('Asia/Kolkata',e.created_at)) due_date,
...
recovery_due as (
    select distinct x.csp_id, x.device_id, x.connection_id, date(convert_timezone('Asia/Kolkata',x.confirmed_at)) due_date
    from (select p:csp_id::string csp_id, p:device_id::string device_id, p:last_connection_id::string connection_id,
                 p:recovery_method::string recovery_method, try_to_timestamp_tz(p:confirmed_at::string) confirmed_at
          from (select try_parse_json(payload) p from csp_asset_custody_service_csp_asset_custody_service.outbox_record
...
bonus_comp as (select distinct csp_id, amount from csp_compensation_service_csp_compensation_service.entitlement_ledger_entries
    where _fivetran_active and entry_type='BONUS_CREDIT'),
bonus_dynamo_single as (select distinct account_id, round(amount,2) amount_rs from DYNAMODB.T_TRANSACTIONS
    where transaction_type in ('WORK_INCENTIVE','DEVICE_INCENTIVE','RATING_INCENTIVE')),
bonus_dynamo_clubbed as (select account_id, round(sum(amount),2) amount_rs from DYNAMODB.T_TRANSACTIONS
    where transaction_type in ('WORK_INCENTIVE','DEVICE_INCENTIVE') group by account_id, date(convert_timezone('Asia/Kolkata',created))),
bonus_events as (
    select 'BONUS_CREDIT' event_type, w.id, w.due_date,
           iff(max(iff(c.csp_id is not null,1,0))=1 or max(iff(ds.account_id is not null,1,0))=1
...
all_events as (
    select event_type, due_date, is_correct from intervention_events
    union all select event_type, due_date, is_correct from recovery_events
    union all select event_type, due_date, is_correct from base_payout_events
    union all select event_type, due_date, is_correct from bonus_events),
period_rates as (
    select e.event_type, p.period_name, round(100.0*sum(e.is_correct)/nullif(count(e.due_date),0),2) rate_pct
