...
classified AS (
    SELECT iw.connection_id, iw.isp_end, iw.cron_time,
        ...
        MIN(CASE WHEN DATE_TRUNC('minute', cp.plan_end) > DATE_TRUNC('minute', iw.isp_end)
                      AND cp.plan_purchased_at < iw.cron_time
                 THEN cp.ROUTER_NAS_ID END) AS nas_pre_cron,
        MIN(CASE WHEN DATE_TRUNC('minute', cp.plan_end) > DATE_TRUNC('minute', iw.isp_end)
                      AND cp.plan_purchased_at >= iw.cron_time
                      AND cp.plan_purchased_at < iw.isp_end
                 THEN cp.ROUTER_NAS_ID END) AS nas_immediate,
        MIN(CASE WHEN cp.plan_purchased_at > iw.isp_end
                 THEN cp.ROUTER_NAS_ID END) AS nas_post_isp
...
all_expected AS (
    SELECT connection_id, isp_end,
        ...
        COALESCE(first_pre_cron, first_immediate, first_post_isp) AS plan_purchased_at,
        COALESCE(nas_pre_cron, nas_immediate, nas_post_isp)       AS router_nas_id
    FROM classified
...
total_match AS (
    SELECT ae.connection_id, ae.isp_end, ae.expected_date, ae.expected_type,
           ae.router_nas_id, ae.plan_purchased_at,
...
unpivoted AS (
    SELECT dt, 1 AS srt, 'Should Create - Total'     AS metric, SUM(expected_cnt)::FLOAT AS val FROM daily GROUP BY dt
    UNION ALL
    SELECT dt, 2,        'Created ±2d - Total',       SUM(matched_total)::FLOAT           FROM daily GROUP BY dt
    UNION ALL
    SELECT dt, 3,        'Should Create - Proactive', expected_cnt::FLOAT                 FROM daily WHERE expected_type='PROACTIVE'
    UNION ALL
    SELECT dt, 4,        'Created ±2d - Proactive',   matched_typed::FLOAT                FROM daily WHERE expected_type='PROACTIVE'
    UNION ALL
    SELECT dt, 5,        'Should Create - Reactive',  expected_cnt::FLOAT                 FROM daily WHERE expected_type='REACTIVE'
    UNION ALL
    SELECT dt, 6,        'Created ±2d - Reactive',    matched_typed::FLOAT                FROM daily WHERE expected_type='REACTIVE'
)
