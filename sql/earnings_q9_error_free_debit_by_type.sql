...
liability_events as (
    ...
    group by w.id, date(convert_timezone('Asia/Kolkata',w.created_at))),
all_events as (
    select event_type, due_date, is_correct from tax_events
    union all select event_type, due_date, is_correct from withdrawal_events
    union all select event_type, due_date, is_correct from netbox_events
    union all select event_type, due_date, is_correct from liability_events),
period_rates as (
